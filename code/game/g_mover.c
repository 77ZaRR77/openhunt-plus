...
qboolean	G_TryPushingEntity( gentity_t *check, gentity_t *pusher, vec3_t move, vec3_t amove ) {
...
#else
	ps = G_GetEntityPlayerState(check);
	if (ps) {
	...
	if (ps) {
		...
		// make sure the view rotates when on a rotating mover
...
#if 1	// JUHOX: non-solid brushes don't push anything
	if (pusher->flags & FL_NON_SOLID) {
		// move the pusher to it's final position
		VectorAdd( pusher->r.currentOrigin, move, pusher->r.currentOrigin );
		VectorAdd( pusher->r.currentAngles, amove, pusher->r.currentAngles );
		VectorCopy(pusher->r.currentOrigin, pusher->s.origin);	// JUHOX
		...
		*obstacle = check;
		...
		for ( p=pushed_p-1 ; p>=pushed ; p-- ) {
...
#else
			{
				...
				ps = G_GetEntityPlayerState(p->ent);
				if (ps) {
					ps->delta_angles[YAW] = p->deltayaw;
					VectorCopy(p->origin, ps->origin);
				}
			}
#endif
...
void G_MoverTeam( gentity_t *ent ) {
	...
	if (part) {
		...
		for ( part = ent ; part ; part = part->teamchain ) {
			...
			VectorCopy(ent->r.currentOrigin, ent->s.origin);	// JUHOX DEBUG
...
static void Touch_DoorTriggerSpectator( gentity_t *ent, gentity_t *other, trace_t *trace ) {
	...
	axis = ent->count;
	// NOTE: the constants below relate to constants in Think_SpawnNewDoorTrigger()
	...
	if (
		fabs(origin[axis] - doorMax) <
		fabs(origin[axis] - doorMin)
	) {
...
void SpawnPlatTrigger( gentity_t *ent ) {
	...
	trigger = G_Spawn();
	if (!trigger) return;	// JUHOX BUGFIX
...
void SP_func_plat (gentity_t *ent) {
	...
	// JUHOX: new keys 'sound1' & 'sound2' for func_plat
#if 0
...
#else
	{
		...
		G_SpawnString("sound1", "sound/movers/plats/pt1_strt.wav", &sound);
		ent->sound1to2 = ent->sound2to1 = G_SoundIndex(sound);

		G_SpawnString("sound2", "sound/movers/plats/pt1_end.wav", &sound);
