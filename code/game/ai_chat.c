...
int BotChat_StartLevel(bot_state_t *bs) {
	...
	//don't chat in teamplay
	if (TeamPlayIsOn()) {
	    //trap_EA_Command(bs->client, "vtaunt");	// JUHOX: no vtaunt available
	    return qfalse;
	}
	// don't chat in tournament mode
	if (gametype == GT_TOURNAMENT) return qfalse;
...
int BotChat_HitTalking(bot_state_t *bs) {
	...
	//if (!lasthurt_client) return qfalse;	// JUHOX: no longer needed
	...
	weap = BotWeaponNameForMeansOfDeath(g_entities[bs->client].client->/*lasthurt_client*/lasthurt_mod);	// JUHOX BUGFIX
	//
...
int BotChat_HitNoKill(bot_state_t *bs) {
	...
	if (BotVisibleEnemies(bs)) return qfalse;
	//
	if (bs->enemy < 0) return qfalse;	// JUHOX: safety check
	if (bs->enemy >= MAX_CLIENTS) return qfalse;	// JUHOX
	BotEntityInfo(bs->enemy, &entinfo);
	if (EntityIsShooting(&entinfo)) return qfalse;
	//
