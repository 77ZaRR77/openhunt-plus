...
static gentity_t* redFlagEntity;	// JUHOX: if dropped or taken
static gentity_t* blueFlagEntity;	// JUHOX: if dropped or taken
...
void Team_CheckDroppedItem( gentity_t *dropped ) {
	if( dropped->item->giTag == PW_REDFLAG ) {
		...
		redFlagEntity = dropped;	// JUHOX
	}
	else if( dropped->item->giTag == PW_BLUEFLAG ) {
		Team_SetFlagStatus( TEAM_BLUE, FLAG_DROPPED );
		blueFlagEntity = dropped;	// JUHOX
	}
...
/*
================
JUHOX: Team_GetDroppedOrTakenFlag
================
*/
gentity_t* Team_GetDroppedOrTakenFlag(int team) {
	...
	switch (team) {
	case TEAM_RED:
		return redFlagEntity;
	case TEAM_BLUE:
		return blueFlagEntity;
	}
	return NULL;
}
...
int Team_TouchEnemyFlag( gentity_t *ent, gentity_t *other, int team ) {
	gclient_t *cl = other->client;
		...
		case TEAM_RED:
			cl->ps.powerups[PW_REDFLAG] = INT_MAX; // flags never expire
			redFlagEntity = other;
			level.ctfRedTakenCount++;
			break;
		case TEAM_BLUE:
			cl->ps.powerups[PW_BLUEFLAG] = INT_MAX; // flags never expire
			blueFlagEntity = other;
			level.ctfBlueTakenCount++;
			break;
		}
#endif
...
================
*/
// JUHOX: SelectRandomTeamSpawnPoint() no longer needed
#if 0
...
gentity_t *SelectRandomTeamSpawnPoint( int teamstate, team_t team ) {
	...
	// JUHOX: use seeded random
#if 0
	selection = rand() % count;
#else
	selection = SeededRandom(GST_playerSpawning) % count;
#endif
