...
==================
*/
bot_state_t* BotAI_IsBot(int client) {
	bot_state_t* bs;

	if (client < 0 || client >= MAX_CLIENTS) return NULL;
	bs = botstates[client];
	if (!bs) return NULL;
	if (!bs->inuse) return NULL;
	return bs;
}
...
/*
==================
JUHOX: BotAI_DebugBot
==================
*/
qboolean BotAI_DebugBot(int clientNum) {
	if (!BotAI_IsBot(clientNum)) return qfalse;

	botstates[clientNum]->debugThisBot ^= 1;
	return qtrue;
}
#endif
...
int BotAI_GetClientState( int clientNum, playerState_t *state ) {
	...
	if (clientNum < 0 || clientNum >= level.num_entities) return qfalse;
...
void BotReportStatus(bot_state_t *bs) {
	...
	// JUHOX: show leader number
#if 0
	if (Q_stricmp(netname, bs->teamleader) == 0) leader = "L";
	else leader = " ";
#else
	{
		static char leaderbuf[32];

		Com_sprintf(leaderbuf, sizeof(leaderbuf), "L%-2d", bs->leader);
		leader = leaderbuf;
	}
#endif
	// JUHOX: special debug info for dead bots
#if 1
	if (bs->cur_ps.stats[STAT_HEALTH] <= 0) {
		BotAI_Print(PRT_MESSAGE, "%-20s         %s : dead\n", netname, leader);
		return;
	}
#endif
	...
	// JUHOX: add some additional info to the bot debug info
#if 1
	{
		char buf[MAX_MESSAGE_SIZE];

		Com_sprintf(buf, sizeof(buf), "%-20s H%-3dA%-3d", netname, bs->cur_ps.stats[STAT_HEALTH], bs->cur_ps.stats[STAT_ARMOR]);
		strcpy(netname, buf);
	}
#endif
	switch(bs->ltgtype) {
		...
		case LTG_CAMPORDER:
		{
#if 0	// JUHOX: also print camp goal
			BotAI_Print(PRT_MESSAGE, "%-20s%s%s: camping\n", netname, leader, flagstatus);
#else
			trap_BotGoalName(bs->teamgoal.number, goalname, sizeof(goalname));
			BotAI_Print(PRT_MESSAGE, "%-20s%s%s: camping at %s\n", netname, leader, flagstatus, goalname);
#endif
			break;
		}
		case LTG_PATROL:
		...
		// JUHOX: report LTG_ESCAPE
#if 1
		case LTG_ESCAPE:
		{
			trap_BotGoalName(bs->teamgoal.number, goalname, sizeof(goalname));
			BotAI_Print(PRT_MESSAGE, "%-20s%s%s: escaping to %s\n", netname, leader, flagstatus, goalname);
			break;
		}
#endif
#if 1	// JUHOX: report LTG_WAIT
		case LTG_WAIT:
		{
		...
		default:
		{
#if 0	// JUHOX: more roaming debug info
...
void BotSetInfoConfigString(bot_state_t *bs) {
	...
	//
	ClientName(bs->client, netname, sizeof(netname));
	if (/*Q_stricmp(netname, bs->teamleader) == 0*/bs->leader == bs->client) leader = "L";	// JUHOX
	else leader = " ";
...
==============
*/
#if 0	// JUHOX: BotTeamLeader() no longer needed
int BotTeamLeader(bot_state_t *bs) {
	int leader;

	leader = /*ClientFromName(bs->teamleader)*/bs->leader;	// JUHOX
...
/*
==============
JUHOX: BotReactionTime

corrects those crazy reaction time values from the bot files
==============
*/
float BotReactionTime(bot_state_t* bs) {
	float reactiontime;

	reactiontime = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_REACTIONTIME, 0, 4);
	reactiontime = 0.1f + 0.05625f * reactiontime * reactiontime;
	if (bs->settings.arenaLord) {
		reactiontime *= 0.8f;
		if (reactiontime > 0.3f) reactiontime = 0.3f;
	}
	return reactiontime;
}
...
/*
==============
JUHOX: BotViewReaction
==============
*/
static void BotViewReaction(bot_state_t* bs, vec3_t perceivedViewDistortion) {
	vec3_t diff;
	float currenttime, reactiontime;
	float upper_timelimit, lower_timelimit, besttime;
	int bestentry, numentries;
	vec3_t avgAngles, avgPrcvAngles;
	float upper_timelimit2, lower_timelimit2;
	int numentries2;
	vec3_t avgAngles2;
	int i;

	currenttime = FloatTime();
	if (bs->viewhistory.lastUpdateTime <= currenttime - 0.02) {
		...
		if (bs->viewhistory.oldestEntry >= VIEWHISTORY_SIZE) bs->viewhistory.oldestEntry = 0;
	}

	reactiontime = 0.5 * bs->reactiontime;
	upper_timelimit = currenttime;
	lower_timelimit = upper_timelimit - 2 * reactiontime;
	/*
	upper_timelimit2 = currenttime - reactiontime;
	lower_timelimit2 = upper_timelimit2 - 2 * reactiontime;
	*/
...
void BotChangeViewAngles(bot_state_t *bs, float thinktime) {
...
#else
	vec3_t perceivedViewDistortion;
	vec3_t viewAnglesDest;
	...
	factor = thinktime / bs->reactiontime;
	if (factor > 1) factor = 1;
	maxchange = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_VIEW_MAXCHANGE, 240, 1800);
	maxchange *= thinktime;
	for (i = 0; i < 2; i++) {
		float viewdelta;
		...
		viewdelta =
			factor *
			AngleSubtract(viewAnglesDest[i], bs->viewhistory.lastviewcommand[i] + perceivedViewDistortion[i]);
		if (viewdelta > maxchange) viewdelta = maxchange;
		else if (viewdelta < -maxchange) viewdelta = -maxchange;
...
#if 0	// JUHOX BUGFIX: ucmd->xxxmove do not use euclidean distance (see PM_CmdScale())
...
#endif
	// JUHOX BUGFIX: since the ucmd->xxxmove fields are of type 'char', it's not recommended to
	// add/subtract 127 (could cause an overflow)
#if 0
...
void BotUpdateInput(bot_state_t *bs, int time, int elapsed_time) {
	bot_input_t bi;
	int j;
	qboolean runningNeeded;	// JUHOX

	//add the delta angles to the bot's current view angles
	for (j = 0; j < 3; j++) {
		bs->viewangles[j] = AngleMod(bs->viewangles[j] + SHORT2ANGLE(bs->cur_ps.delta_angles[j]));
	}
#if 1	// JUHOX: don't change the view during railgun jump
	if (bs->railgunJump_jumptime) {
...
#if !RESPAWN_DELAY	// JUHOX: don't use respawn hack with auto-respawn
	//respawn hack
	if (bi.actionflags & ACTION_RESPAWN) {
		if (bs->lastucmd.buttons & BUTTON_ATTACK) bi.actionflags &= ~(ACTION_RESPAWN|ACTION_ATTACK);
	}
#endif
#if 1	// JUHOX: we depend on some elements of the playerState, which is not updated every frame, so do it now
	if (!BotAI_GetClientState(bs->client, &bs->cur_ps)) goto CreateUserCommand;
#endif
#if RESPAWN_DELAY	// JUHOX: press the attack button if dead, to respawn at a regular spawn point
	if (bs->cur_ps.stats[STAT_HEALTH] <= 0) {
		if (bi.actionflags & ACTION_RESPAWN) {
		...
		bs->preventJump = qfalse;
		bs->specialMove = qfalse;
		bs->forceWalk = qfalse;
		bs->walkTrouble = qfalse;
		bs->railgunJump_ordertime = 0;
		bs->railgunJump_jumptime = 0;
		goto CreateUserCommand;
	}
#endif
#if 1	// JUHOX: delay release of the machinegun a bit
	if (bs->weaponnum == WP_MACHINEGUN) {
		if (bi.actionflags & ACTION_ATTACK) {
			bs->machinegun_attack_time = FloatTime();
		}
		else if (bs->machinegun_attack_time > FloatTime() - 1) {
			bi.actionflags |= ACTION_ATTACK;
		}
	}
	else {
		bs->machinegun_attack_time = 0;
	}
#endif
#if 1	// JUHOX: running needed?
	runningNeeded = qfalse;
	if ((bi.actionflags & ACTION_WALK) || bs->forceWalk) {
		trace_t trace;
		vec3_t start;
		vec3_t end;

		VectorMA(bs->cur_ps.origin, 0.150 * g_speed.value, bi.dir, start);
...
#endif
#if 1	// JUHOX: make the 'preventJump' flag work
	if (bs->preventJump && !runningNeeded && !bs->specialMove) {
		bi.actionflags &= ~ACTION_JUMP;
	}
#endif
#if 1	// JUHOX: make the 'forceWalk' flag work
	if (
		runningNeeded ||
	...
	if (bi.actionflags & ACTION_WALK) {
		...
		if (
			...
			NOT (
				bs->ltgtype &&
				DistanceSquared(bs->origin, bs->teamgoal.origin) < 50
			)
		) {
			bs->walkTrouble = qtrue;
		}
	}
#endif
#if 1	// JUHOX: make the railgun jump work
	if (bs->railgunJump_ordertime) {
		bs->walkTrouble = qfalse;
		// already jumped?
		if (bs->railgunJump_jumptime > 0) {
			if (bs->railgunJump_jumptime > FloatTime() - 0.25) {
				bi.actionflags &= ~(ACTION_ATTACK | ACTION_MOVEFORWARD | ACTION_MOVEBACK | ACTION_MOVELEFT | ACTION_MOVERIGHT);
				bi.speed = 0;
			}
			else if (bs->cur_ps.weaponTime <= 0) {
				bi.actionflags |= ACTION_ATTACK;
			}
			else {
				bs->railgunJump_ordertime = 0;
		...
		// railgun requirements still met?
		else if (bs->railgunJump_ordertime > FloatTime() - 3 && bs->cur_ps.ammo[WP_RAILGUN] > 0) {
			// weapon ready for jump?
			if (bs->cur_ps.weapon == WP_RAILGUN && bs->cur_ps.weaponTime <= 0) {
				if (
					(bi.actionflags & ACTION_JUMP) ||
					(
						bs->cur_ps.groundEntityNum != ENTITYNUM_NONE &&
						bs->railgunJump_ordertime < FloatTime() - 2 &&
						bs->cur_ps.viewangles[PITCH] >= 85
					)
				) {
					bi.actionflags &= ~ACTION_ATTACK;
					bi.actionflags |= ACTION_JUMP;
					bs->railgunJump_jumptime = FloatTime();
				}
			}
		}
		else {
			// railgun jump failed, abort
			bs->railgunJump_ordertime = 0;
			bs->railgunJump_jumptime = 0;
		}
	}
#endif
#if 1	// JUHOX: don't let the bot run at low speed
	if (bi.speed <= 200) bi.actionflags |= ACTION_WALK;
#endif
	CreateUserCommand:	// JUHOX
...
void BotScheduleBotThink(void) {
	...
	for( i = 0; i < MAX_CLIENTS; i++ ) {
		...
		// JUHOX BUGFIX: initialize 'botthink_residual' to zero. we use a fairer schedule in 'BotAIStartFrame()'
#if 0
		botstates[i]->botthink_residual = bot_thinktime.integer * botnum / numbots;
		botnum++;
#else
		botstates[i]->botthink_residual = 0;
#endif
	}
}
...
int BotAISetupClient(int client, struct bot_settings_s *settings, qboolean restart) {
	...
	memset(&bs->humanHelpers, -1, sizeof(bs->humanHelpers));	// JUHOX
	bs->leader = -1;	// JUHOX
	...
	bs->reactiontime = BotReactionTime(bs);	// JUHOX
...
//		trap_Cvar_Set("bot_report", "0");
	...
	if ( botlib_residual >= thinktime ) {
		...
		for (i = 0; i < MAX_GENTITIES; i++) {
			...
			// JUHOX: do not update armor fragments in the bot library
#if 1
			if (
				ent->s.eType == ET_ITEM &&
	...
	for( i = 0; i < MAX_CLIENTS; i++ ) {
		...
		//
		// JUHOX BUGFIX: randomize the bot schedule
		// JUHOX: faster AI for fast bots
#if 0
...
#else
		{
			...
			/*
			avgThinkTime = bot_thinktime.integer;
			t = thinktime;
			if (botstates[i]->reactiontime < 0.2) {
				avgThinkTime = avgThinkTime >> 1;
				t = t >> 1;
			}
			*/
			avgThinkTime = 500.0 * botstates[i]->reactiontime;
			t = thinktime;

			if (botstates[i]->botthink_residual + (rand() % avgThinkTime) >= t) {
				botstates[i]->botthink_residual -= t;

				if (!trap_AAS_Initialized()) return qfalse;

				if (g_entities[i].client->pers.connected == CON_CONNECTED) {
					BotAI(i, (float) t / 1000);
				}
			}
		}
#endif
