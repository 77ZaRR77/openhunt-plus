...
/*
================
JUHOX: CG_GetScreenCoordinates

Coordinates are 640*480 virtual values
Needs cg.refdef.vieworg, cg.refdef.viewaxis, and cg.refdef.fov_x/y set
returns qfalse if origin is behind the player
=================
*/
qboolean CG_GetScreenCoordinates(const vec3_t origin, float* x, float* y) {
	vec3_t dir;
	vec3_t projection;
	float f;

	VectorSubtract(origin, cg.refdef.vieworg, dir);
	projection[0] = DotProduct(dir, cg.refdef.viewaxis[0]);
	if (projection[0] < 1) return qfalse;
	projection[1] = DotProduct(dir, cg.refdef.viewaxis[1]);
	projection[2] = DotProduct(dir, cg.refdef.viewaxis[2]);

	f = 320.0 / tan(cg.refdef.fov_x * (M_PI / 360.0));
	*x = 320.0 - f * projection[1] / projection[0];

	f = 240.0 / tan(cg.refdef.fov_y * (M_PI / 360.0));
	*y = 240.0 - f * projection[2] / projection[0];
...
void CG_DrawChar( int x, int y, int width, int height, int ch ) {
	...
	if (ah >= 12) {
		...
		charsetShader = cgs.media.charsetShaders[((ch&8)>>3) + ((ch&192)>>5)];
