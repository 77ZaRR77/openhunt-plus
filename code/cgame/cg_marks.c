...
/*
===============
JUHOX: CG_AddNearboxFace
===============
*/
static void CG_AddNearboxFace(qhandle_t shader, const vec3_t origin, const vec3_t dir1, const vec3_t dir2) {
	polyVert_t verts[4];
	...
	VectorAdd(origin, dir1, verts[1].xyz);
	verts[1].st[0] = 0;
	verts[1].st[1] = 1;
	...
	VectorAdd(origin, dir1, verts[2].xyz);
	VectorAdd(verts[2].xyz, dir2, verts[2].xyz);
	verts[2].st[0] = 0;
	verts[2].st[1] = 0;
	...
	VectorAdd(origin, dir2, verts[3].xyz);
	verts[3].st[0] = 1;
	verts[3].st[1] = 0;
...
===============
*/
#define NEARBOX_SIZE 10000
void CG_AddNearbox(void) {
	vec3_t origin;
	vec3_t dir1;
	vec3_t dir2;

	if (!cgs.nearboxShaderName[0]) return;
	if (!cg_nearbox.integer) return;	

	/*
	VectorSet(dir1, 0, NEARBOX_SIZE, 0);
	VectorSet(dir2, -NEARBOX_SIZE, 0, 0);
	VectorCopy(cg.refdef.vieworg, origin);
	origin[1] -= 0.5 * NEARBOX_SIZE;
	origin[0] += 0.5 * NEARBOX_SIZE;
	origin[2] += 0.5 * NEARBOX_SIZE;
	CG_AddNearboxFace(cgs.media.nearbox_up, origin, dir1, dir2);
	*/
	...
	origin[0] -= 0.5 * NEARBOX_SIZE;
	origin[2] -= 0.5 * NEARBOX_SIZE;
	CG_AddNearboxFace(cgs.media.nearbox_dn, origin, dir1, dir2);

	VectorSet(dir1, 0, NEARBOX_SIZE, 0);
	VectorSet(dir2, 0, 0, NEARBOX_SIZE);
	VectorCopy(cg.refdef.vieworg, origin);
	origin[1] -= 0.5 * NEARBOX_SIZE;
	origin[2] -= 0.5 * NEARBOX_SIZE;
	origin[0] += 0.5 * NEARBOX_SIZE;
	CG_AddNearboxFace(cgs.media.nearbox_ft, origin, dir1, dir2);
	...
	origin[2] -= 0.5 * NEARBOX_SIZE;
	origin[0] -= 0.5 * NEARBOX_SIZE;
	CG_AddNearboxFace(cgs.media.nearbox_bk, origin, dir1, dir2);

	VectorSet(dir1, -NEARBOX_SIZE, 0, 0);
	VectorSet(dir2, 0, 0, NEARBOX_SIZE);
	VectorCopy(cg.refdef.vieworg, origin);
	origin[0] += 0.5 * NEARBOX_SIZE;
	origin[2] -= 0.5 * NEARBOX_SIZE;
	origin[1] += 0.5 * NEARBOX_SIZE;
	CG_AddNearboxFace(cgs.media.nearbox_lf, origin, dir1, dir2);
	...
	origin[2] -= 0.5 * NEARBOX_SIZE;
	origin[1] -= 0.5 * NEARBOX_SIZE;
	CG_AddNearboxFace(cgs.media.nearbox_rt, origin, dir1, dir2);
}
...
void CG_AddLightningMarks(int numMarks) {
	...
	for (i = 0; i < numMarks; i++) {
		...
		*/
		...
		VectorMA(start, 8000, dir, end);

		CG_Trace(&trace, start, NULL, NULL, end, -1, CONTENTS_SOLID);
		if (trace.fraction < 0.01 || trace.fraction >= 1) continue;
...
#if SPECIAL_VIEW_MODES	// JUHOX: variables for light blobs
#define MAX_LIGHT_BLOBS 256
#define LIGHTBLOB_FADE_TIME 1000	// msec
typedef struct {
	float x, y;
	float radiusX;
	float radiusY;
...
static int oldestLightBlob;
static lightBlob_t lightBlobs[MAX_LIGHT_BLOBS];
#endif
...
void AddLightBlob(float x, float y, float radius, const vec3_t color, qboolean fastFade) {
	lightBlob_t* blob;

	blob = &lightBlobs[oldestLightBlob];
	oldestLightBlob = (oldestLightBlob + 1) & (MAX_LIGHT_BLOBS - 1);

	blob->x = x * cgs.screenXScale;
	blob->y = y * cgs.screenYScale;
	blob->radiusX = radius * cgs.screenXScale;
	blob->radiusY = radius * cgs.screenYScale;
	VectorCopy(color, blob->color);
	blob->startTime = cg.time;
	if (fastFade) {
		blob->fadeTime = 250;
	}
	else {
		blob->fadeTime = (int) (radius * 2);
	}
}
#endif
...
/*
===============
JUHOX: CG_CheckStrongLight
===============
*/
#if SPECIAL_VIEW_MODES
void CG_CheckStrongLight(const vec3_t origin, float intensity, const vec3_t color) {
	float zoomFactor;
	float distance;
	float maxDistance;
	float x, y;
	...
	if (cg.viewMode != VIEW_amplifier) return;
	...
	zoomFactor = 90.0 / cg.refdef.fov_x;
	intensity *= zoomFactor;
	distance = Distance(cg.refdef.vieworg, origin);
	maxDistance = 10.0 * intensity;
	if (distance >= maxDistance) return;

	if (!CG_GetScreenCoordinates(origin, &x, &y)) return;
	if (x < 0 || x >= 640 || y < 0 || y >= 480) return;

	CG_SmoothTrace(&trace, cg.refdef.vieworg, NULL, NULL, origin, -1, CONTENTS_SOLID);
	if (trace.fraction < 1) return;

	intensity *= 1 - distance / maxDistance;

	AddLightBlob(x, y, intensity, color, fastFade);
}
#endif

/*
===============
JUHOX: CG_DrawLightBlobs
===============
*/
#if SPECIAL_VIEW_MODES
void CG_DrawLightBlobs(void) {
	int i;

	if (cg.viewMode != VIEW_amplifier) return;

	for (i = 0; i < MAX_LIGHT_BLOBS; i++) {
		lightBlob_t* blob;
		vec4_t color;
		float x, y;
		float rx, ry;
		...
		blob = &lightBlobs[i];
		if (!blob->startTime) continue;
		if (cg.time > blob->startTime + blob->fadeTime) {
			blob->startTime = 0;
			continue;
		}

		color[0] = blob->color[0];
		color[1] = blob->color[1];
		color[2] = blob->color[2];
		color[3] = 1.0f - (float)(cg.time - blob->startTime) / (float)(blob->fadeTime);
		...
		rx = blob->radiusX;
		ry = blob->radiusY;
		trap_R_DrawStretchPic(x - rx, y - ry, 2 * rx, 2 * ry, 0, 0, 1, 1, cgs.media.bfgLFGlareShader);
	}
