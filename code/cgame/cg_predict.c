...
void CG_PredictPlayerState( void ) {
	...
	usercmd_t	oldestCmd;
	usercmd_t	latestCmd;
	playerState_t* predictionSource;	// JUHOX
	...
	if ( cg.nextSnap && !cg.nextFrameTeleport && !cg.thisFrameTeleport ) {
		...
		cg.predictedPlayerState = cg.snap->ps;
		predictionSource = &cg.snap->ps;	// JUHOX
	...
	for ( cmdNum = current - CMD_BACKUP + 1 ; cmdNum <= current ; cmdNum++ ) {
		...
		if ( cg.predictedPlayerState.stats[STAT_TARGET] >= 0 &&	cg.predictedPlayerState.stats[STAT_TARGET] < ENTITYNUM_MAX_NORMAL ) {
			...
			target = &cg_entities[cg.predictedPlayerState.stats[STAT_TARGET]];
			BG_EvaluateTrajectory(&target->currentState.pos, cg.time, cg_pmove.target);
