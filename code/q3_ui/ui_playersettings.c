...
static void PlayerSettings_DrawName( void *self ) {
	...
	Q_CleanStr( name );
	// JUHOX: don't use proportional font, we want the new charset
#if 0
	UI_DrawProportionalString( 320, 440, name, UI_CENTER|UI_BIGFONT, text_color_normal );
#else
	UI_DrawString(320, 420, name, UI_CENTER|UI_GIANTFONT, text_color_normal);
#endif
}
...
static void PlayerSettings_SaveChanges( void ) {
	...
	trap_Cvar_SetValue( "color2", uitogamecode[s_playersettings.effects.curvalue] );	// JUHOX
}
