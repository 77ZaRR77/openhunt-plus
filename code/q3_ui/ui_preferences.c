...
typedef struct {
	...
	menuradiobutton_s	glassCloaking;	// JUHOX
	menuradiobutton_s	lensFlare;		// JUHOX
	menuradiobutton_s	bfgSuperExpl;	// JUHOX
...
static void Preferences_SetMenuItems( void ) {
	...
	s_preferences.bfgSuperExpl.curvalue		= trap_Cvar_VariableValue("cg_bfgSuperExpl") != 0;	// JUHOX
	s_preferences.autoglc.curvalue			= trap_Cvar_VariableValue("cg_autoglc") != 0;	// JUHOX
}
...
static void Preferences_MenuInit( void ) {
	...
	// JUHOX: adjust menu position to attend for 5 new entries
#if 0
	y = 144;
#else
	y = 144 - 2.5 * (BIGCHAR_HEIGHT+2);
#endif
	...
	// JUHOX: init "glass cloaking" menu field
#if 1
	y += BIGCHAR_HEIGHT+2;
	s_preferences.glassCloaking.generic.type		= MTYPE_RADIOBUTTON;
	s_preferences.glassCloaking.generic.name		= "Glass Cloaking:";
	s_preferences.glassCloaking.generic.flags		= QMF_PULSEIFFOCUS|QMF_SMALLFONT;
	s_preferences.glassCloaking.generic.callback	= Preferences_Event;
	s_preferences.glassCloaking.generic.id			= ID_GLASSCLOAKING;
	s_preferences.glassCloaking.generic.x			= PREFERENCES_X_POS;
	s_preferences.glassCloaking.generic.y			= y;
#endif

	// JUHOX: init "lens flare" menu field
#if 1
	y += BIGCHAR_HEIGHT+2;
	s_preferences.lensFlare.generic.type		= MTYPE_RADIOBUTTON;
	s_preferences.lensFlare.generic.name		= "Lens Flare:";
	s_preferences.lensFlare.generic.flags		= QMF_PULSEIFFOCUS|QMF_SMALLFONT;
	...
	// JUHOX: init "bfg super explosion" menu field
#if 1
	y += BIGCHAR_HEIGHT+2;
	s_preferences.bfgSuperExpl.generic.type		= MTYPE_RADIOBUTTON;
	s_preferences.bfgSuperExpl.generic.name		= "High Quality BFG Explosion:";
	s_preferences.bfgSuperExpl.generic.flags	= QMF_PULSEIFFOCUS|QMF_SMALLFONT;
	s_preferences.bfgSuperExpl.generic.callback	= Preferences_Event;
	s_preferences.bfgSuperExpl.generic.id		= ID_BFGSUPEREXPL;
	s_preferences.bfgSuperExpl.generic.x		= PREFERENCES_X_POS;
	s_preferences.bfgSuperExpl.generic.y		= y;
#endif
	...
	s_preferences.musicmode.generic.flags		= QMF_PULSEIFFOCUS|QMF_SMALLFONT;
	s_preferences.musicmode.generic.callback	= Preferences_Event;
	s_preferences.musicmode.generic.id			= ID_MUSICMODE;
	...
	s_preferences.autoglc.generic.flags		= QMF_PULSEIFFOCUS|QMF_SMALLFONT;
	s_preferences.autoglc.generic.callback	= Preferences_Event;
	...
	Menu_AddItem( &s_preferences.menu, &s_preferences.glassCloaking );	// JUHOX
	Menu_AddItem( &s_preferences.menu, &s_preferences.lensFlare);	// JUHOX
	Menu_AddItem( &s_preferences.menu, &s_preferences.bfgSuperExpl);	// JUHOX
